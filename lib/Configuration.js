...
class ConfigNode {
  ...
  constructor(parent, key, options) {
    ...
    if (options && options.type === 'object' && options.properties)
  ...
  define(key, def) {
    ...
    if (def.description)
      sch.description = def.description;
    if (def.env)
      sch.env = def.env;
    if (def.arg)
      sch.arg = def.arg;
  ...
  defineAll(defs) {
    ...
    for (const k of Object.keys(defs)) {
      ...
      /* istanbul ignore next */
        throw new SchemaError('Invalid definition in "%s". One of the "format" or "properties" properties must be set', k);
    }
    return this;
  }
  ...
  getSchema() {
    const iterate = (node, target) => {
      merge(target, node._schema, {deep: true});
    ...
    const parse = (sch, value) => {
      try {
        ...
        throw new ValidationError('Validation error for "%s" property. ' +
            '"%s" does not matches required format. ' + e.message, key, value);
      }
      throw new ValidationError('Validation error for "%s" property. ' +
          '"%s" does not matches required format', key, value);
    };
...
class Configuration extends ConfigNode {
   ...
   */
  load(obj) {
    merge(this._values, obj, {deep: true});
    return this;
  }
